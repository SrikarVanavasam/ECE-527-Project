...
void my_prj_accelerator(accelerator_input_t* x, accelerator_output_t* score){
#pragma HLS INTERFACE m_axi port = x depth = 12 offset = slave
#pragma HLS INTERFACE m_axi port = score depth = 2 offset = slave
#pragma HLS INTERFACE s_axilite port = return bundle = control
  int n_f = n_features;
  int n_c = BDT::fn_classes(n_classes);
  for(int n = 0; n < N; n++){
    #pragma HLS pipeline
    input_arr_t x_int;
